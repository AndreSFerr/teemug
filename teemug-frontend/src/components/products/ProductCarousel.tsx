...
type Props = {
  title: string;
  products?: Product[]; // Torna opcional para prevenir crash
  carouselId: string;
};

const ProductCarousel = ({ title, products = [], carouselId }: Props) => {
  // Só agrupa se houver produtos
  const groupProducts = products.length
    ? products.reduce<Product[][]>((acc, curr, index) => {
